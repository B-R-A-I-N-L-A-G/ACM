...
OPEN_AND_RUN_INPUTS
    ...
    cin >> t;
    for (int i = 0; i < t; ++i) {
        int size, firecracker_count, hooligan_pos, guard_pos;
        cin >> size >> firecracker_count >> hooligan_pos >> guard_pos;

        vector<int> firecrackers (firecracker_count);
        cin >> firecrackers;
        rng::sort(firecrackers);

        int cracker_count = std::abs(hooligan_pos - guard_pos) - 1;
        int count = 0, time = hooligan_pos < guard_pos ? guard_pos - 1 : size - guard_pos;
        for (auto x : rng::reverse_view(firecrackers)){
            if (count == cracker_count) break;
            if(x < time) {
                count++;
                time--;
            }
