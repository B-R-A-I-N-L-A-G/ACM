...
OPEN_AND_RUN_INPUTS
    ...
    for (int i = 0; i < t; ++i) {
        ...
        int suresure = false;
        if (last >= 120) suresure++;
        if( last >= 240) suresure++;
        for (int j = 0; j < count - 1; ++j) {
            int num;
            cin >> num;
            if (num - last >= 120) suresure++;
            if(num - last >= 240) suresure++;
            last = num;
        }

        if (1440 - last >= 120) suresure++;
        if(1440 - last >= 240) suresure++;
        cout << (suresure >=2? "YES":"NO") << endl;
    }
